...
```{r simulateNonProportionalHazards,eval=FALSE}
...
for(k in 1:obs)
{
  dat <- simulGomCensNonProportional(N=1000, lambda=0.5,gamma=0.5, beta=betas,timeBetas, rateC=0.1)
  fit <- coxph(Surv(time, status) ~ x.1+x.2+x.3+x.4+xt.1*time+xt.2*time+xt.3*time+xt.4*time, data=dat)
